...
function generateResFile(resources) {
  const lines = ['# Generated by md-novel-builder', 'TILESET novel_sjis_font "novel/font/misaki_gothic.png" NONE NONE'];
...
function generateDataSource(resources, compiled, pool, settings, profile, budget, variables) {
...
function generateProject(snapshot) {
  ...
  return {
    files: {
      ...
      'src/generated/novel_data.c': generateDataSource(resources, compiled, pool, snapshot.sceneDocument.settings || {}, snapshot.targetProfile, budget, variables),
    },
    warnings,
    report: {
