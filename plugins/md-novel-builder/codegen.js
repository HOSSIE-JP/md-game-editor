...
function compileScenes(sceneDocument, catalog, bindings, resources, pool, warnings, variables, options = {}) {
  ...
  const messageSeparateTop = new Set(options.messageSeparateTop || []);
  ...
  for (const [sceneIndex, scene] of sceneDocument.scenes.entries()) {
    ...
    for (const [commandIndex, command] of emitted.commands.entries()) {
      ...
      if (command.type === 'background') {
        ...
        messages.push({ symbol, array, pageSymbols, speaker, color: mdColor(textColor), mouthSlot: command.mouthSlot == null ? -1 : clamp(command.mouthSlot, 0, 3, 0), layoutFlags: messageSeparateTop.has(`${sceneIndex}:${commandIndex}`) ? 'NOV_MSG_SEPARATE_TOP' : '0' });
        data = `&${symbol}`;
        ...
        choices.push({
          ...
          options,
          variableIndex,
          defaultIndex: clamp(command.defaultIndex, 0, Math.max(0, options.length - 1), 0),
          layoutFlags: choiceLowered.has(`${sceneIndex}:${commandIndex}`) ? 'NOV_CHOICE_LOWERED' : '0',
        });
        data = `&${symbol}`;
...
function generateDataSource(resources, compiled, pool, settings, profile, budget, variables, fontPlan) {
  const messageSource = compiled.messages.map((message) => `static const u8* const ${message.array}[] = { ${message.pageSymbols.join(', ')} };\nstatic const NovelMessage ${message.symbol} = { ${message.speaker}, ${message.array}, ${message.pageSymbols.length}, ${message.mouthSlot}, 0x${message.color.toString(16).padStart(4, '0')}, ${message.layoutFlags} };`).join('\n');
  ...
  const choiceSource = compiled.choices.map((choice) => `static const NovelChoice ${choice.symbol} = { ${choice.options.length}, ${choice.defaultIndex}, ${choice.layoutFlags}, ${choice.variableIndex}, { ${choice.options.map((option) => `{ ${option.label}, ${option.targetScene}, ${option.value} }`).join(', ')} } };`).join('\n');
...
function cloneVisualState(state) {
  return {
    ...
    messageOverlay: state.messageOverlay ? JSON.parse(JSON.stringify(state.messageOverlay)) : null,
...
function chunkCount(glyphs) {
  return Math.ceil(Math.max(0, Number(glyphs) || 0) / 2);
}

function messagePageHardware(command, pageText, separateTop = false) {
  const rows = String(pageText || '').split('\n').slice(0, 4);
  while (rows.length < 4) rows.push('');
  const lengths = rows.map((line, index) => Math.min(index === 3 ? 18 : 19, Array.from(line).length));
  const chunks = lengths.map(chunkCount);
  const speakerLength = Math.min(16, Array.from(String(command.speaker || '')).length);
  const speakerChunks = chunkCount(speakerLength);
  const topChunks = Math.max(speakerChunks, chunks[0]);
  const middleChunks = Math.max(chunks[1], chunks[2]);
  const pieces = separateTop
    ? Math.max(
      speakerChunks + chunks[0],
      speakerChunks + chunks[0] + chunks[1] + chunks[2],
      speakerChunks + chunks[0] + middleChunks + chunks[3] + 1,
    )
    : Math.max(
      speakerChunks + chunks[0],
      topChunks + chunks[1] + chunks[2],
      topChunks + middleChunks + chunks[3] + 1,
    );
  return {
    kind: 'message',
    pieces,
    separateTop,
    vramTiles: NOVEL_MESSAGE_VRAM_TILES,
    revealFrames: 3,
    lines: [
      { y: 136, pieces: separateTop ? speakerChunks : topChunks, pixels: (separateTop ? speakerLength : Math.max(speakerLength, lengths[0])) * 16 },
      { y: 152, pieces: separateTop ? chunks[0] : topChunks, pixels: (separateTop ? lengths[0] : Math.max(speakerLength, lengths[0])) * 16 },
      { y: 168, pieces: middleChunks, pixels: Math.max(lengths[1], lengths[2]) * 16 },
      { y: 184, pieces: middleChunks, pixels: Math.max(lengths[1], lengths[2]) * 16 },
      { y: 200, pieces: chunks[3] + 1, pixels: lengths[3] * 16 + 8 },
    ],
  };
}

function messageHardwareStates(command, separateTop = false) {
  return paginateText(command.text).map((page) => messagePageHardware(command, page, separateTop));
}

function choiceHardware(command, lowered = false) {
  const lengths = (command.choices || []).slice(0, 4).map((choice) => Math.min(17, Array.from(String(choice?.label || '')).length));
  while (lengths.length < 4) lengths.push(0);
  const chunks = lengths.map(chunkCount);
  const topChunks = Math.max(chunks[0], chunks[1]);
  const middleChunks = Math.max(chunks[2], chunks[3]);
  return {
    kind: 'choice',
    lowered,
    pieces: topChunks + middleChunks + 1,
    vramTiles: NOVEL_MESSAGE_VRAM_TILES,
    revealFrames: 2,
    lines: [
      { y: 136 + (lowered ? 16 : 0), pieces: topChunks + 1, pixels: Math.max(lengths[0], lengths[1]) * 16 + 8 },
      { y: 152 + (lowered ? 16 : 0), pieces: topChunks + 1, pixels: Math.max(lengths[0], lengths[1]) * 16 + 8 },
      { y: 168 + (lowered ? 16 : 0), pieces: middleChunks + 1, pixels: Math.max(lengths[2], lengths[3]) * 16 + 8 },
      { y: 184 + (lowered ? 16 : 0), pieces: middleChunks + 1, pixels: Math.max(lengths[2], lengths[3]) * 16 + 8 },
    ],
  };
}

function spriteTextIntersectsShadow(entry) {
  if (!entry) return false;
  let y = numberOrZero(entry.y);
  for (const line of String(entry.text || '').split(/\r?\n/)) {
    if (Array.from(line).length && y + 16 > 128 && y < 224) return true;
    y += 16;
  }
  return false;
}

function visibleBudget(sceneDocument, bindings, options = {}) {
  const diagnostics = [];
  const messageColorFallbacks = new Set();
  const messageSeparateTop = new Set();
  const configuredChoiceLowered = new Set(options.choiceLowered || []);
  const choiceLowered = new Set(configuredChoiceLowered);
  ...
  let maxScanlinePiecesState = null;
  let maxScanlinePixelsState = null;
  let maxMessagePieces = 0;
  ...
  const measure = (state) => {
    ...
    const messageOverlay = state.messageOverlay;
    const owners = [];
    ...
    if (messageOverlay) {
      for (let slotIndex = 0; slotIndex < metas.length; slotIndex += 1) {
        const meta = metas[slotIndex];
        if ((meta.yMax + (meta.frameHeight || 0)) <= 128) continue;
        const forbidden = meta.palette === 'PAL3' ? [14, 15] : [14];
        const used = Array.isArray(meta.paletteIndicesUsed) ? meta.paletteIndicesUsed.map(Number) : [];
        const invalid = forbidden.filter((index) => used.includes(index));
        if (invalid.length) recordPaletteDiagnostic({
          severity: 'error',
          code: 'shadow-operator-palette-index',
          palette: meta.palette,
          assetIds: [meta.assetId],
          message: `${meta.assetId} uses H/S operator palette index ${invalid.join(',')} while intersecting y>=128. Reconvert it with the shadow-safe profile.`,
        }, state, `${meta.assetId}:${invalid.join(',')}`);
      }
      for (let slotIndex = 0; slotIndex < state.spriteTexts.length; slotIndex += 1) {
        if (!spriteTextIntersectsShadow(state.spriteTexts[slotIndex])) continue;
        recordPaletteDiagnostic({
          severity: 'error',
          code: 'spritetext-shadow-overlap',
          message: `SpriteText slot ${slotIndex} intersects the H/S message band at y>=128. Move or hide it before Message/Choice.`,
        }, state, `spritetext:${slotIndex}`);
      }
    }
    const spriteTiles = metas.reduce((sum, meta) => sum + (meta.maxNumTile || 0), 0);
    const actorSpritePieces = metas.reduce((sum, meta) => sum + (meta.maxNumSprite || 0), 0);
    const spritePieces = actorSpritePieces + (messageOverlay?.pieces || 0);
    ...
    maxMessagePieces = Math.max(maxMessagePieces, messageOverlay?.pieces || 0);
    for (let line = 0; line < 224; line += 1) {
      ...
      for (const messageLine of messageOverlay?.lines || []) {
        if (line < messageLine.y || line >= messageLine.y + 16) continue;
        pieces += messageLine.pieces;
        pixels += messageLine.pixels;
      }
      if (pieces > maxScanlinePieces) {
        maxScanlinePieces = pieces;
        maxScanlinePiecesState = { sceneId: String(rawScenes[state.sceneIndex]?.id || ''), commandIndex: Math.max(0, state.pc - 1), scanline: line };
      }
      if (pixels > maxScanlinePixels) {
        maxScanlinePixels = pixels;
        maxScanlinePixelsState = { sceneId: String(rawScenes[state.sceneIndex]?.id || ''), commandIndex: Math.max(0, state.pc - 1), scanline: line };
      }
    }
    measurements.push({
      sceneIndex: state.sceneIndex,
      ...
      overlayTiles,
      windowVisible: state.windowVisible,
      messageVramTiles: messageOverlay?.vramTiles || 0,
    });
  };
  const blank = {
  ...
  for (let head = 0; head < queue.length && !overflow; head += 1) {
    ...
    if (command.type === 'background') {
      ...
      const actorPixelsAtSpeaker = state.slots.filter(Boolean).reduce((sum, slot) => {
        const metadata = bindings.assets?.[slot.assetId]?.metadata || {};
        const height = Number(metadata.frameHeight) || 0;
        const width = Number(metadata.frameWidth) || 0;
        return (136 >= slot.yMin && 136 < slot.yMax + height) ? sum + width : sum;
      }, 0);
      const mergedHardware = messageHardwareStates(command, false);
      const separateTop = mergedHardware.some((hardware) => actorPixelsAtSpeaker + (hardware.lines[0]?.pixels || 0) > 320);
      if (separateTop) messageSeparateTop.add(`${state.sceneIndex}:${state.pc}`);
      const hardwareStates = separateTop ? messageHardwareStates(command, true) : mergedHardware;
      for (const hardware of hardwareStates) {
        const displayed = cloneVisualState(next);
        displayed.windowVisible = true;
        displayed.messageColor = String(command.textColor || '');
        displayed.messageOverlay = hardware;
        measure(displayed);
      }
      ...
      const choiceKey = `${state.sceneIndex}:${state.pc}`;
      let lowered = configuredChoiceLowered.has(choiceKey);
      if (!options.choiceLayoutFixed && !lowered) {
        const pressure = (hardware) => hardware.lines.reduce((peak, messageLine) => {
          let actorPieces = 0;
          let actorPixels = 0;
          for (const slot of state.slots.filter(Boolean)) {
            const metadata = bindings.assets?.[slot.assetId]?.metadata || {};
            const height = Number(metadata.frameHeight) || 0;
            const width = Number(metadata.frameWidth) || 0;
            if (messageLine.y < slot.yMin || messageLine.y >= slot.yMax + height) continue;
            actorPieces += Math.ceil(width / 32);
            actorPixels += width;
          }
          const pieces = actorPieces + messageLine.pieces;
          const pixels = actorPixels + messageLine.pixels;
          return Math.max(peak, Math.max(0, pixels - 320) * 100 + Math.max(0, pieces - 20));
        }, 0);
        const normalPressure = pressure(choiceHardware(command, false));
        const loweredPressure = pressure(choiceHardware(command, true));
        if (normalPressure > 0 && loweredPressure < normalPressure) {
          lowered = true;
          choiceLowered.add(choiceKey);
        }
      }
      next.windowVisible = true;
      next.messageOverlay = choiceHardware(command, lowered);
      measure(next);
  ...
  return {
    ...
    choiceLowered: [...choiceLowered].sort(),
    diagnostics,
  };
}
...
function generateProject(snapshot) {
  ...
  const compiled = compileScenes(snapshot.sceneDocument, snapshot.catalog, snapshot.bindings, resources, pool, warnings, variables, {
    ...
    messageSeparateTop: budget.messageSeparateTop,
    choiceLowered: budget.choiceLowered,
  });
  return {
    files: {
