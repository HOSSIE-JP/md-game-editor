  ...
  for (const [sceneIndex, scene] of sceneDocument.scenes.entries()) {
    ...
    for (const [commandIndex, command] of emitted.commands.entries()) {
      ...
      if (command.type === 'background') {
        ...
        messages.push({ symbol, array, pageSymbols, speaker, color: mdColor(command.textColor), mouthSlot: command.mouthSlot == null ? -1 : clamp(command.mouthSlot, 0, 3, 0), layoutFlags: messageSeparateTop.has(`${sceneIndex}:${commandIndex}`) ? 'NOV_MSG_SEPARATE_TOP' : '0' });
        data = `&${symbol}`;
...
function messagePageHardware(command, pageText, separateTop = false) {
  ...
  return {
    kind: 'message',
    pieces: speakerLength + lengths.reduce((sum, value) => sum + value, 0) + 1,
    separateTop,
    vramTiles: NOVEL_MESSAGE_VRAM_TILES,
    revealFrames: 4,
    lines: [
      { y: 136, pieces: speakerLength, pixels: speakerLength * 16 },
      { y: 152, pieces: lengths[0], pixels: lengths[0] * 16 },
      { y: 168, pieces: lengths[1], pixels: lengths[1] * 16 },
      { y: 184, pieces: lengths[2], pixels: lengths[2] * 16 },
      { y: 200, pieces: lengths[3] + 1, pixels: lengths[3] * 16 + 8 },
    ],
  };
}
...
function choiceHardware(command, lowered = false) {
  ...
  return {
    kind: 'choice',
    lowered,
    pieces: lengths.reduce((sum, value) => sum + value, 0) + 1,
    vramTiles: NOVEL_MESSAGE_VRAM_TILES,
    revealFrames: 3,
    lines: [
      { y: 136 + (lowered ? 16 : 0), pieces: lengths[0] + 1, pixels: lengths[0] * 16 + 8 },
      { y: 152 + (lowered ? 16 : 0), pieces: lengths[1] + 1, pixels: lengths[1] * 16 + 8 },
      { y: 168 + (lowered ? 16 : 0), pieces: lengths[2] + 1, pixels: lengths[2] * 16 + 8 },
      { y: 184 + (lowered ? 16 : 0), pieces: lengths[3] + 1, pixels: lengths[3] * 16 + 8 },
    ],
  };
}
...
function visibleBudget(sceneDocument, bindings, options = {}) {
  ...
  const measure = (state) => {
    ...
    if (messageOverlay) {
      const messagePalette = state.background?.palette === 'PAL1' ? 'PAL2' : 'PAL1';
      const conflictingMessageOwners = (ownersByPalette.get(messagePalette) || []).filter((owner) => {
        const used = bindings.assets?.[owner.assetId]?.metadata?.paletteIndicesUsed;
        return Array.isArray(used) && used.map(Number).includes(14);
      });
      if (conflictingMessageOwners.length) {
        const assetIds = [...new Set(conflictingMessageOwners.map((owner) => owner.assetId))].sort();
        recordPaletteDiagnostic({
          severity: 'error',
          code: 'message-normal-index-conflict',
          palette: messagePalette,
          assetIds,
          message: `${messagePalette} index 14 is reserved for normal-intensity message glyphs while the message window is visible (${assetIds.join(', ')}). Reconvert or assign those assets to another palette.`,
        }, state, `${messagePalette}:${assetIds.join(',')}`);
      }
