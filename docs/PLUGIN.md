...
Wizardry 型の薄い壁を持つ 3D ダンジョンをフロア単位で編集します。最大 20x20 の可変サイズ、壁/扉/一方通行/ダークゾーン/宝箱/上り階段/下り階段/開始位置を扱い、指定サイズから玄室、扉、宝箱、離れた階段を含むランダム生成を行えます。v1.1 では壁・扉・床・天井の4要素を再利用可能な素材セットとして複数定義し、フロア編集時の素材セット選択を即座に3Dプレビューへ反映します。宝箱・上り階段・下り階段の3要素はデシジョンタイルへ焼き込まないビルボードスプライトであり素材セットに紐付ける技術的理由がないため、`settings.common_assets` としてプロジェクトに1組だけ定義し、全ての素材セット・フロアで共有します。階段セルは宝箱と同様に壁ではなく通行可能なイベントセルとして中央にビルボード表示され、前進/後退でそのセルへ足を踏み入れると自動的に対応するフロア (上り階段なら1つ上、下り階段なら1つ下) へ遷移し、遷移先の対応する種類の階段セルそのものへ壁で塞がれていない最初の向きを向いて到着します。プレイヤーが立っているセルに宝箱/階段がある場合はビルボードのカリングを行わず最至近スプライトへ固定表示するため、足元に何があるか常に視認できます。
