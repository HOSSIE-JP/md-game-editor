...
壁・扉と重なるビルボード (宝箱・階段・エネミー) は、BG_AのタイルPriorityで遮蔽します。共有コアは4bit深度コード (`0=壁なし、1～15=遠→近`、`clamp(16 - ceil(z * 3), 1, 15)`) を各8x8壁タイル内の最小非ゼロ値へ縮約します。表示中ビルボードの外接タイル範囲と比較し、重なる全ビルボードについて `tile_min_wall_depth > billboard_depth` の場合だけ壁タイルを高Priorityにします。同一深度・遠い壁は低Priorityのままなので誤遮蔽せず、混在深度タイルでは小さな隠し損ねを許容します。左右旋回・前後移動・エネミーのセル間スライドでは、座標・距離バケットと同時に深度コードも整数補間し、3Dプレビューも「BG_B → 低Priority壁 → ビルボード → 高Priority壁」の順で合成します。エネミーが壁を跨ぐ場合、現在セルがLOS外でも直前セルがLOS内かつ両端の焼き込みポーズが有効なら、スライド終端直前まで描画候補を保持します。壁Priorityが補間位置を順に覆い、終端で現在セルのLOSへ確定するため、壁から現れる方向と壁へ消える方向のどちらも滑らかになります。

フロア編集の「敵」ツールでエネミーのスポーン地点を配置できます (1フロア8体まで、宝箱/階段セルとは排他、`cell.enemy` → flags bit 0x10)。エネミーの見た目は共通素材 `enemy_texture` (192×96 = 4方向列×歩行2フレーム行の48×48グリッド、透過可、未設定時は手続き生成のプレースホルダー) から、8距離バケット×8行 (方向×歩行) の384×384スプライトシートとして1回だけ焼き込みます。3Dプレビューは実機と同一のAI (16bit xorshift RNGによる決定的な徘徊 = 75%直進+候補一様選択、正面直線3マスの視界検知 — 壁と扉が遮る — で追跡モードへ移行し軸差優先の貪欲移動、見失って5歩で徘徊復帰) をリアルタイムにシミュレートします。扉はエネミーの視界だけでなく移動も遮り、プレイヤーだけが通過できるため、扉で区切った部屋の中へエネミーを留められます。エネミーはプレイヤー/宝箱/上り階段/下り階段/他エネミーのセルへ侵入できず、未正規化の旧データに競合フラグがあっても宝箱・階段上にはスポーンしません。接触時のステータス表示を含むシミュレーション状態は、踏破状態と同じ規則でセッション内フロア別に保持されます。
...
Priority用のstatic/fwd/turnデシジョンテーブルはテクスチャ非依存の共通資産として `res/dungeon/generated/priority/bake.bin` に1組だけキャッシュします。葉は0～15の深度値のみで、深度PNGや `TILESET dun_occlusion_depth_tiles` は生成しません。キャッシュキーは `animation_frames`、`turn_frames`、`render-core.js` のバージョンだけで、素材セット・共通素材・移動速度の変更では複製・再生成されません。生成予算にはPriorityテーブルのノード数・値数・ROM推定量を計上します。
...
ビルボードはSprite Engineの自動VRAM割当・自動タイル転送を使う低Priorityスプライトとして描画します。選択中 `AnimationFrame` の実メタスプライト外接タイル範囲を集め、壁タイルのPriority bitだけを動的に更新します。通常の移動・旋回では既存BG_AマップDMAへ含め、敵スライド中は外接タイル範囲・depth_code・表示数・壁世代が変わり、かつPriority bitが実際に変化したときだけ400ワードのマップを再送します。静止視点の可視候補・LOS・焼き込みポーズ検索結果は最大8件の `DunBillboardPlan` として敵リスト世代単位でキャッシュし、毎フレームの全42候補セル再走査を避けます。各スロットは直前に適用したdefinition・座標・animation/frame・表示状態を保持し、値が変わったsetterだけを呼びます。`DUN_refreshBillboards()` はスプライト状態が変わった場合にだけ `SPR_update()` を実行するため、メインループから無条件にSprite Attribute Tableを再構築・DMAしません。スプライト画素マスク、固定VRAM、9216B RAMバッファ、手動スプライトDMAは使用しません。
