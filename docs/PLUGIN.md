...
ビルボードはSprite Engineの自動VRAM割当・自動タイル転送を使う低Priorityスプライトとして描画します。選択中 `AnimationFrame` の実メタスプライト外接タイル範囲を集め、壁タイルのPriority bitだけを動的に更新します。通常の移動・旋回では既存BG_AマップDMAへ含め、敵スライド中は外接タイル範囲・depth_code・表示数・壁世代が変わり、かつPriority bitが実際に変化したときだけ400ワードのマップを再送します。スプライト画素マスク、固定VRAM、9216B RAMバッファ、手動スプライトDMAは使用しません。
