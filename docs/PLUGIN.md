...
Wizardry 型の薄い壁を持つ 3D ダンジョンをフロア単位で編集します。最大 20x20 の可変サイズ、壁/扉/一方通行/ダークゾーン/宝箱/上り階段/下り階段/開始位置を扱い、指定サイズから玄室、扉、宝箱、離れた階段を含むランダム生成を行えます。

3D プレビューと SGDK 用タイルパターン焼き込みは、共有レンダリングコア `render-core.js` (UMD 形式、renderer は `import()`、service は `require()` で同一ファイルを読み込む) を通ります。コアは f=100 (水平 FOV 約 90°) の透視投影ラスタライザで奥行き 4 マス・横方向は奥行き連動 ±2 の視界を描画し、16 色量子化パレット・深度シェード帯の床/天井・壁+中央パネル方式の扉を含めてプレビューと実機出力がピクセル一致 (WYSIWYG) になります。プレビューの移動/旋回は実機と同じ離散アニメフレーム (前進 `animation_frames`、旋回 `turn_frames`、左旋回は右旋回の鏡像) を再生し、宝箱/階段ビルボードも実機と同じ座標テーブルと LOS ルール (セル中心間の supercover 線分判定。壁・扉・階段セルが視線を遮る) で合成します。階段セルはソリッドで、開いた面は壁として描かれ、その上に階段アイコンのビルボードが重なります。プレビューで階段セルへ前進すると実機同様にフロアを移動します (上り→order-1 フロアの下り階段の隣、下り→order+1 フロアの上り階段の隣)。テクスチャは 3x2 アトラス (旧形式) または扉タグを含む 4x2 アトラス + サイドカー `<atlas>.json` (`{"columns":4,"rows":2}`) を参照し、未定義タグは手続き生成にフォールバックします。

データは project-local の `data/dungeon/floors/*.json` と `data/dungeon/settings.json` に保存し、保存/ビルド時に `inc/dungeon_data.h`、`src/dungeon_data.c`、`inc/dungeon_patterns.h`、`src/dungeon_patterns.c` へ生成します。`dungeon_patterns.c` には「画面タイル (8x8) ごとの周囲エッジ状態 (開/壁/扉) → タイル実体」のデシジョンツリー (静止 1 + 前進 N-1 + 右旋回 M-1 フレーム分)、エッジ定義テーブル、ビルボード座標テーブル、暗闇用パレット `dun_palette_dark` を出力します。タイル実体は H/V フリップ正規化で重複排除して `res/dungeon/generated/dungeon_view_tileset.png` に、宝箱/階段のスケール別スプライトシートを `dungeon_bb_*.png` に書き出し、`res/resources.res` へ `PALETTE` x2 / `TILESET` / `SPRITE` x3 を自動登録します (旧 `dungeon_view_map.png` と `TILEMAP` は廃止)。焼き込みはテクスチャ+設定のハッシュでキャッシュされ、変更がない保存では再生成をスキップします。エクスポート結果にはタイル数・テーブルサイズの予算統計と警告 (`budget` / `warnings`) が含まれます。
...
ダンジョンゲーム用の SGDK エンジンテンプレートを同期し、ダンジョンエディターのフロアデータから ROM 用 C データと ResComp アセットを生成します。ゲーム側の描画は BG_A 上の 25x16 タイル（200x128px）で、エンジン (`dungeon_view.c`) は毎アニメフレーム、エディターが焼き込んだデシジョンツリーを周囲エッジ状態 (開/壁/扉) で評価して 400 タイルを解決し、ROM タイルセットから RAM ステージングへ集めて VRAM のダブルバッファ (400 タイル x2 バンク) へ 2 vblank 分割 DMA (`VDP_loadTileData` + `VDP_setTileMapDataRect`, `DMA_QUEUE`) で転送します。十字キーは押しっぱなしで連続動作し、上/下で前後移動 (前進 `DUN_FWD_FRAMES` フレーム、後退は移動先セル基準の逆再生)、左/右で旋回 (`DUN_TURN_FRAMES` フレームの本物の回転アニメーション、左旋回は右旋回テーブルの鏡像評価 + HFLIP 合成) を行います。階段セルはソリッドで、前進バンプ (押した瞬間のみ) でフロアを移動します (`DUN_ACTION_STAIRS`)。宝箱/上り階段/下り階段は PAL1 のプリスケール済みビルボードスプライト (`SPR` エンジン、supercover 線分 LOS で表示判定 — 壁・扉・階段セルの向こうは非表示) として滑らかに移動し、暗闇セルでは `dun_palette_dark` へのパレット差し替えで減光します。ビュー右の余白には BG_A + PAL2 で 4px/セルのミニマップ (壁/扉/階段/宝箱/プレイヤー位置と向き) を常時表示します。テンプレート素材として壁・床・天井・宝箱・上り階段・下り階段のテクスチャアトラスを同梱します (扉テクスチャは 4x2 アトラス + サイドカー導入までは手続き生成フォールバック)。
