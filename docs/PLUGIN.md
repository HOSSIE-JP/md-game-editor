...
Wizardry 型の薄い壁を持つ 3D ダンジョンをフロア単位で編集します。最大 20x20 の可変サイズ、壁/扉/一方通行/ダークゾーン/宝箱/上り階段/下り階段/開始位置を扱い、指定サイズから玄室、扉、宝箱、離れた階段を含むランダム生成を行えます。v1.1 では壁・扉・床・天井・宝箱・上り階段・下り階段の7要素を再利用可能な素材セットとして複数定義し、フロア編集時の素材セット選択を即座に3Dプレビューへ反映します。

素材セットは `data/dungeon/settings.json` の順序付き `asset_sets` に `{ id, name, assets }` として保存し、各 `data/dungeon/floors/*.json` は `asset_set_id` だけを参照します。IDはセットの表示名を変更しても変わらない安定IDで、セットは1～255件、ID重複と存在しない参照は保存・ビルドエラーです。最後のセット、およびフロアから参照中のセットは削除できません。旧フロアのinline `assets` は読み込み・ビルド時に同じ内容をまとめた仮想セットとして扱い、最初の明示保存時に `asset_sets` / `asset_set_id` 形式へ一括移行します。フロアと設定を同時に確定する場合は `saveDungeonState({ floor, settings })` を使用し、exportを1回だけ行います。既存の個別保存hookも後方互換のため利用できます。

素材カードの「選択/置換」はPNG/BMPのファイルダイアログから標準 `image-import-pipeline.convertToIndexed16()` を呼び、結果の `targetExtension` を使って `res/dungeon/textures/<set-id>/` へ保存します。新規のタグなし画像は画像全体、既存の `path#tag` は3x2/4x2アトラスの該当要素として読み取ります。変換後の保存形式は8bit・非interlace・16色以下のindexed PNGです。壁/扉は96x96・不透明、床/天井は32x32・不透明、宝箱/上り階段/下り階段は48x48・透過可で、rendererとserviceの両方が寸法・色数・透過・project-local保存先を検証します。不正な素材は設定へ反映しません。各カードにはcontain表示のピクセルプレビュー、寸法、色数、保存先、検証結果を表示し、上書き時はtexture cacheを無効化します。

3DプレビューとSGDK生成は共有レンダリングコア `render-core.js` を通ります。床/天井はデシジョンタイルへ焼き込まず、それぞれ32x32パターンをBG_Bの下半分/上半分（各200x64）へ固定反復配置します。壁/扉は非描画部をpalette index 0にした透明BG_Aの動的デシジョンタイルとして重ねるため、固定背景が透けて見えます。コアはこの「固定BG_B + 透明BG_A」を合成し、奥行き4マスの3Dプレビュー、離散移動/旋回アニメーション、宝箱/階段ビルボード、supercover LOS、暗闇を実機出力と一致させます。

保存/ビルド時は参照されている素材セットごとに壁/扉タイルセット、床/天井の32x64背景タイルセット、通常/暗闇palette、ビルボードpaletteと3種のsprite sheet、デシジョンテーブルを生成します。フロアデータは素材セットindexを持ち、生成側の `DunViewSet` レジストリから対応リソースを選びます。焼き込みcache、タイル数、テーブル容量、ROM推定量、警告はセット別と合計で管理し、未参照セットはROMへ出力しません。`resources.res` の生成ブロックはセットID由来の安全なsymbolで冪等更新します。
...
ダンジョンゲーム用のSGDKエンジンテンプレートを同期し、ダンジョンエディターのフロアデータからROM用CデータとResCompアセットを生成します。`DUN_applyViewSet()` は初期表示とフロア遷移時に `DungeonFloorData` の素材セットindexから `DunViewSet` を選び、PAL0/PAL1、固定BG_B、active tileset/frame table、ビルボード定義、暗闇paletteをまとめて切り替えます。暗闇中に遷移した場合も移動先セットの暗色paletteを適用します。

エンジン (`dungeon_view.c`) はBG_A上の25x16タイル（200x128px）について、毎アニメフレーム、壁/扉だけを焼き込んだデシジョンツリーを周囲エッジ状態（開/壁/扉）で評価し、透明indexを含む400タイルをVRAMのダブルバッファへ2 vblank分割DMAで転送します。床/天井はBG_Bへ固定反復配置され、BG_Aの透明部から表示されます。十字キーの連続動作、前後移動、旋回、階段によるフロア移動、宝箱/階段のLOS付きビルボード、暗闇、BG_A + PAL2のミニマップは従来どおり動作し、フロア遷移時には選択素材セットも同時に切り替わります。
